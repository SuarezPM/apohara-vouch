...
- **AI/ML API (Claude Fable 5 / Claude Sonnet 4.5)** — Multimodal reasoning backend. **50+ real AI/ML API calls per end-to-end demo run** across the Extractor, Fraud Auditor, and GAAP Classifier agents (Claude Fable 5 family, anthropic/claude-sonnet-4.5 dispatch). These are the high-judgment calls: PDF-to-JSON extraction, fraud pattern detection, GAAP line-item classification. The Fable 5 system-prompt cache hits ~95% across runs, dropping the marginal cost to near zero on repeated invocations. Provider: `AIMLAPIBackend` in `themis-orchestrator`.
...
| AI/ML API | Multimodal reasoning (extractor, fraud auditor, GAAP classifier) | 50+ real calls per demo run (Claude Fable 5 / Sonnet 4.5) |
